...
class NavdataManager {
        ...
        for (const line of lines) {
            ...
            if (inAirspaceSection) {
                ...
                if (line.startsWith('BORDER:')) {
                    ...
                    currentSector.borders = parts.map((item) => Number(item.replace('\r', '')))
                }
    ...
    public async generateNavdata(packageId: string, namespace: string, eseFilePath: string, outputPath: string): Promise<void> {
        ...
        await system.writeFile(`${outputPath}/${packageId}/datasets/navdata.json`, JSON.stringify(navdata))
    }
    ...
    public async generateDataSets(packageId: string, sctData: SCT, eseData: ESE, ignoreTypes: string[], outputPath: string): Promise<string[]> {
        ...
        features = features.filter(f => {
            if (f.properties.type === 'geo') {
                ...
                if (f.properties.section.toLowerCase().includes('political boundaries')) {
                    return false
                }
        ...
        if (allTypes.length > 1) {
            ...
            datasets
                .forEach(async t => {
                    await this.generateGeoJsonFilesForType(path, t, features.filter(f => f.properties.type === t))
                })
        }
