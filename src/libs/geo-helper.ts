...
export class GeoHelper {
  public convertESEGeoCoordinatesToCartesian(
    ...
    try {
      ...
      return toMercator([coord.getLongitude(), coord.getLatitude()]);
    } catch (error) {
      return null;
    }
  }
  ...
  private reformatCoordinates(coord: string): string {
    ...
    return `${Number(parts[0].substring(1, 4))}:${parts[1]}:${parts[2]}.${
      parts[3]
  ...
  public convertGeoCoordinatesToESE(latStr: string, lonStr: string): string {
    ...
    const convLongStr = `${nsew}${this.formatESEDegrees(d)}.${this.formatESEMin(
      m
    )}.${this.formatESESec(s)}`;
    let [dl, ml, sl, nsewl] = latitude;
    const convLatStr = `${nsewl}${this.formatESEDegrees(
      dl
    )}.${this.formatESEMin(ml)}.${this.formatESESec(sl)}`;
